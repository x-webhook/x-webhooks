...
func newTestPayload(timestamp time.Time) *testPayload {
	...
	wh, _ := xwebhook.NewWebhook(tp.secret)
...
func TestWebhook(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			modifyPayload: func(tp *testPayload) {
				...
				unbrandedHeaders.Set("webhook-id", tp.header.Get("xwebhook-id"))
				unbrandedHeaders.Set("webhook-timestamp", tp.header.Get("xwebhook-timestamp"))
				unbrandedHeaders.Set("webhook-signature", tp.header.Get("xwebhook-signature"))
		...
		},
		{
			...
			testPayload: newTestPayload(time.Now()),
			modifyPayload: func(tp *testPayload) {
				tp.header.Del("xwebhook-timestamp")
			},
			expectedErr: true,
		},
		{
			...
			testPayload: newTestPayload(time.Now()),
			modifyPayload: func(tp *testPayload) {
				tp.header.Del("xwebhook-signature")
			},
			expectedErr: true,
		},
		{
			...
			testPayload: newTestPayload(time.Now()),
			modifyPayload: func(tp *testPayload) {
				tp.header.Set("xwebhook-signature", "v1,Ceo5qEr07ixe2NLpvHk3FH9bwy/WavXrAFQ/9tdO6mc=")
			},
			expectedErr: true,
		},
		{
			...
			testPayload: newTestPayload(time.Now()),
			modifyPayload: func(tp *testPayload) {
				tp.header.Set("xwebhook-signature", "v1,")
			},
			expectedErr: true,
		},
		{
		...
		},
		{
			...
			modifyPayload: func(tp *testPayload) {
				sigs := []string{
					...
					tp.header.Get("xwebhook-signature"), // valid signature
					"v1,Ceo5qEr07ixe2NLpvHk3FH9bwy/WavXrAFQ/9tdO6mc=",
				}
				tp.header.Set("xwebhook-signature", strings.Join(sigs, " "))
			},
			expectedErr: false,
		},
		{
		...
		},
		{
			...
			testPayload: newTestPayload(time.Now()),
			modifyPayload: func(tp *testPayload) {
				tp.header.Set("xwebhook-timestamp", fmt.Sprint(time.Date(2000, 1, 1, 0, 0, 0, 0, time.UTC).Unix()))
			},
			noEnforceTimestamp: true,
			expectedErr:        true,
		},
	...
	for _, tc := range testCases {
		...
		wh, err := xwebhook.NewWebhook(tc.testPayload.secret)
		if err != nil {
			t.Error(err)
			continue
		}
...
func TestWebhookPrefix(t *testing.T) {
	...
	wh, err := xwebhook.NewWebhook(tp.secret)
	if err != nil {
		t.Fatal(err)
	}
	...
	wh, err = xwebhook.NewWebhook(fmt.Sprintf("whsec_%s", tp.secret))
	if err != nil {
		t.Fatal(err)
	}
...
func TestWebhookSign(t *testing.T) {
	...
	wh, err := xwebhook.NewWebhook(key)
	if err != nil {
		t.Fatal(err)
	}
