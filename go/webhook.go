...
// Verify validates the payload against the xwebhook signature headers
...
// VerifyIgnoringTimestamp validates the payload against the xwebhook signature headers
// using the webhooks signing secret.
//
// Returns an error if the body or headers are missing/unreadable
// or if the signature doesn't match.
...
func (wh *Webhook) verify(payload []byte, headers http.Header, enforceTolerance bool) error {
	msgId := headers.Get("xwebhook-id")
	msgSignature := headers.Get("xwebhook-signature")
	msgTimestamp := headers.Get("xwebhook-timestamp")
