...
class WebhookTest {
    ...
    fun verifyValidBrandlessPayloadAndheader() {
        ...
        unbrandedHeaders["webhook-id"] = testPayload.headerMap["xwebhook-id"]!!
        unbrandedHeaders["webhook-timestamp"] = testPayload.headerMap["xwebhook-timestamp"]!!
        unbrandedHeaders["webhook-signature"] = testPayload.headerMap["xwebhook-signature"]!!
    ...
    fun verifyValidPayloadWithMultipleSignaturesIsValid() {
        ...
        val sigs =
            arrayOf(
                ...
                // valid signature
                testPayload.headerMap["xwebhook-signature"]!![0],
                "v1,Ceo5qEr07ixe2NLpvHk3FH9bwy/WavXrAFQ/9tdO6mc=",
            )
        testPayload.headerMap["xwebhook-signature"] = ArrayList(listOf(sigs.joinToString(" ")))
        val webhook = Webhook(testPayload.secret)
        webhook.verify(testPayload.payload, testPayload.headers())
    }
    ...
    fun verifySignatureWithDifferentVersionThrowsException() {
        val testPayload = TestPayload(System.currentTimeMillis())
        testPayload.headerMap[Webhook.XWEBHOOK_MSG_ID_KEY] = ArrayList(listOf("v2,g0hM9SsE+OTPJTGt/tmIKtSyZlE3uFJELVlNIOLJ1OE="))
        assertFailsWith<WebhookVerificationException>(
            block = {
                verify(testPayload)
            },
    ...
    fun verifyMissingPartsInSignatureThrowsException() {
        val testPayload = TestPayload(System.currentTimeMillis())
        testPayload.headerMap[Webhook.XWEBHOOK_MSG_ID_KEY] = ArrayList(listOf("invalid_signature"))
        assertFailsWith<WebhookVerificationException>(
            block = {
                verify(testPayload)
            },
    ...
    fun verifySignatureMismatchThrowsException() {
        val testPayload = TestPayload(System.currentTimeMillis())
        testPayload.headerMap[Webhook.XWEBHOOK_MSG_ID_KEY] = ArrayList(listOf("v1,invalid_signature"))
        assertFailsWith<WebhookVerificationException>(
            block = {
                verify(testPayload)
            },
    ...
    private inner class TestPayload(timestampInMS: Long) {
        ...
        init {
            ...
            headerMap = HashMap()
            headerMap["xwebhook-id"] = ArrayList(listOf(id))
            headerMap["xwebhook-timestamp"] = ArrayList(listOf(timestamp))
            headerMap["xwebhook-signature"] = ArrayList(listOf(String.format("v1,%s", signature)))
        }
    }
