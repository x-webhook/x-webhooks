...
from xwebhook.webhooks import hmac_data, Webhook, WebhookVerificationError
...
def test_timestamp_raises_error():
    testPayload = PayloadForTesting()
    del testPayload.header["xwebhook-timestamp"]
...
def test_missing_signature_raises_error():
    testPayload = PayloadForTesting()
    del testPayload.header["xwebhook-signature"]
...
def test_invalid_signature_raises_error():
    testPayload = PayloadForTesting()
    testPayload.header["xwebhook-signature"] = (
...
def test_valid_unbranded_signature_is_valid_and_returns_json():
    testPayload = PayloadForTesting()
    unbrandedHeaders = {
        "webhook-id": testPayload.header.get("xwebhook-id"),
        "webhook-signature": testPayload.header.get("xwebhook-signature"),
        "webhook-timestamp": testPayload.header.get("xwebhook-timestamp"),
    }
...
def test_multi_sig_payload_is_valid():
    ...
    testPayload.header["xwebhook-signature"] = " ".join(sigs)
