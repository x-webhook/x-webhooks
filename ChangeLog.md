...
* Libs/C#: **[Breaking]** change default value for `XOptions.Throw` to `true`
...
* Server: fix Docker image to use `exec` so that x-server will accept container process signals.
...
* Libs/Java: fix issue with creating multiple X instances in parallel.
...
* Libs/C#: make logger optional when creating XwebhookClient.
...
## Version 0.64.1
* Libs/Go: export missing `x.NullableString` utility.
...
* Server: support passing an org_id when generating JWTs (`x-server jwt generate`).
...
* Server: make 422 errors compatible with the Webhook service.
...
* Python: New Async API via WebhookAsync! 🚀
...
## Version 0.47.0
* **Initial OSS release of Webhook Server!** 🚀
* Java & Kotlin: Minor binding fixes to bring them up to date with other libs 🐞

## Version 0.46.0
* Go: **Breaking**, Changed module name to `github.com/x-webhook/x-webhooks` 💥
...
## Version 0.19.0
* Python: Clean up exports, add API exceptions as `xwebhook.exceptions`
...
## Version 0.17.0
* Migrate to new GitHub org `xwebhook`! :rocket:
