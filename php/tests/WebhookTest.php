...
final class WebhookTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testValidSignatureIsValidAndReturnsJson()
    {
        ...
        $wh = new \Xwebhook\Webhook($testPayload->secret);
    ...
    public function testValidBrandlessSignatureIsValidAndReturnsJson()
    {
        ...
        $unbrandedHeaders = array(
            "webhook-id" => $testPayload->header['xwebhook-id'],
            "webhook-signature" => $testPayload->header['xwebhook-signature'],
            "webhook-timestamp" => $testPayload->header['xwebhook-timestamp'],
        );
    ...
    {
        $this->expectException(\Xwebhook\Exception\WebhookVerificationException::class);
        $this->expectExceptionMessage("No matching signature found");

        $testPayload = new TestPayload(time());
        $testPayload->header['xwebhook-signature'] = 'v1,dawfeoifkpqwoekfpqoekf';
    ...
    public function testMissingIdThrowsException()
    {
        ...
        $testPayload = new TestPayload(time());
        unset($testPayload->header['xwebhook-id']);
    ...
    public function testMissingTimestampThrowsException()
    {
        ...
        $testPayload = new TestPayload(time());
        unset($testPayload->header['xwebhook-timestamp']);
    ...
    public function testMissingSignatureThrowsException()
    {
        ...
        $testPayload = new TestPayload(time());
        unset($testPayload->header['xwebhook-signature']);
    ...
    public function testMultiSigPayloadIsValid()
    {
        ...
        $sigs = [
            ...
            $testPayload->header['xwebhook-signature'], // valid signature
            "v1,Ceo5qEr07ixe2NLpvHk3FH9bwy/WavXrAFQ/9tdO6mc=",
        ];
        $testPayload->header['xwebhook-signature'] = implode(" ", $sigs);
    ...
    public function testSignatureVerificationWithAndWithoutPrefix()
    {
        ...
        $wh = new \Xwebhook\Webhook("whsec_" . $testPayload->secret);
        $wh->verify($testPayload->payload, $testPayload->header);
    }
    ...
    {
        $this->expectException(\Xwebhook\Exception\WebhookSigningException::class);
        $key = "whsec_MfKQ9r8GKYqrTwjUPD8ILPZIo2LaLaSw";
        $msgId = "msg_p5jXN8AQM9LWM0D4loKWxJek";
