...
use xwebhook_bridge_types::{SenderInput, TransformerJob};
use xwebhook_ksuid::{KsuidLike as _, KsuidMs};
...
#[derive(Parser)]
pub struct Args {
    #[arg(long, env = "XWEBHOOK_BRIDGE_CFG_FILE", help = "Path to the config file.")]
    cfg_file: Option<PathBuf>,
    #[arg(
        long,
        env = "XWEBHOOK_BRIDGE_CFG",
...
async fn main() -> Result<()> {
    ...
    if let Some(fp) = args.cfg_file {
        ...
        for name in ["xwebhook-bridge.yaml", "xwebhook-bridge.yml", "xwebhook-bridge.json"] {
