...
use xwebhook_bridge_types::xwebhook::webhooks::Webhook;
...
#[derive(Clone)]
pub struct XwebhookVerifier {
    webhook: Arc<Webhook>,
}

impl XwebhookVerifier {
    pub fn new(webhook: Arc<Webhook>) -> Self {
        Self { webhook }
    }
}

impl std::fmt::Debug for XwebhookVerifier {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("XwebhookVerifier").finish()
    }
}

#[async_trait]
impl VerificationMethod for XwebhookVerifier {
    /// This [`VerificationMethod::validate`] implementation *requires* that the headers *and* payload
    /// be in their byte representations due to the requirements of the [`xwebhook`] library. Please lazily
    ...
    async fn validate(&self, req: SerializableRequest<Unvalidated>) -> Result<bool> {
        ...
        match (headers, payload) {
            ...
            _ => {
                anyhow::bail!("`XwebhookVerifier::validate` given string representations")
            }
        }
    }
}
...
mod tests {
    ...
    use super::{super::types::SerializableRequest, XwebhookVerifier, VerificationMethod};

    #[tokio::test]
    async fn test_xwebhook_verification() {
        ...
        let sv = XwebhookVerifier { webhook };

        let req = http::request::Request::builder()
            .method("POST")
            .uri("test.uri")
            .header("xwebhook-id", "msg_valid")
            .header("xwebhook-signature", signature.clone())
            .header("xwebhook-timestamp", &format!("{timestamp}"))
        ...
        let req = http::request::Request::builder()
            .method("POST")
            .uri("test.uri")
            .header("xwebhook-id", "msg_invalid")
            .header("xwebhook-signature", signature)
            .header("xwebhook-timestamp", &format!("{timestamp}"))
            .body(axum::body::Full::new(payload))
            .unwrap();
