...
use xwebhook_bridge_types::{
    xwebhook, ReceiverInputOpts, ReceiverOutput, TransformationConfig, TransformerTx, WebhookVerifier,
};

use super::verification::{NoVerifier, XwebhookVerifier, VerificationMethod, Verifier};
        ...
        for cfg in routes {
            let verifier = match &cfg.input {
                ReceiverInputOpts::Webhook {
                    verification: WebhookVerifier::Xwebhook { endpoint_secret },
                    ..
                }
                | ReceiverInputOpts::XwebhookWebhook {
                    endpoint_secret, ..
                } => XwebhookVerifier::new(Arc::new(
                    xwebhook::webhooks::Webhook::new(endpoint_secret).expect("Invalid Xwebhook secret"),
                ))
                .into(),
                ReceiverInputOpts::Webhook {
