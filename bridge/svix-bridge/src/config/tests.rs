...
use xwebhook_bridge_plugin_queue::config::{QueueInputOpts, RabbitMqInputOpts};
use xwebhook_bridge_types::{SenderOutputOpts, XwebhookSenderOutputOpts};
...
const OMNIBUS: &str = r#"
# Xwebhook Bridge Example Configuration
...
opentelemetry:
  ...
  sample_ratio: 0.5
  # Optional: default "xwebhook-bridge"
...
# Senders consume JSON from their input, optionally transform, then forward to Xwebhook.
# Currently supported inputs are various message queue consumers while the sole
# output is "xwebhook" (which does a Create Message API request)
senders:
  ...
  - name: "gcp-example"
    ...
    # Optional - when unset, messages from the queue will be sent to Xwebhook as-is.
    transformation: |
    ...
    output:
      type: "xwebhook"
      # Required (the Xwebhook token to use when creating messages with this consumer)
      token: "XYZ"
...
fn test_sender_parses_ok() {
    let conf: Result<WebhookSenderConfig, _> = serde_yaml::from_str(
        r#"
name: "from-rabbit-local-to-xwebhook"
input:
    type: "rabbitmq"
    queue_name: "local"
...
fn test_receivers_example() {
    let fp = concat!(
        ...
        "/../xwebhook-bridge.example.receivers.yaml"
    );
...
fn test_senders_example() {
    let fp = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../xwebhook-bridge.example.senders.yaml"
    );
    let conf: Config = serde_yaml::from_slice(&std::fs::read(fp).unwrap()).unwrap();
    assert!(!conf.senders.is_empty());
    assert!(conf.receivers.is_empty());
...
fn test_variable_substitution_repeated_lookups() {
    ...
    vars.insert(String::from("XWEBHOOK_TOKEN"), String::from("x"));
    let cfg = Config::from_src(src, Some(&vars)).unwrap();
    ...
    if let WebhookSenderConfig {
        ...
        output: SenderOutputOpts::Xwebhook(XwebhookSenderOutputOpts { token, .. }),
        ..
    } = &cfg.senders[0]
    {
