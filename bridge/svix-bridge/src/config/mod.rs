...
use xwebhook_bridge_plugin_queue::config::{QueueInputOpts, QueueOutputOpts};
...
pub struct Config {
    /// Config for reading messages from plugins and forwarding to Xwebhook.
    #[serde(default)]
...
impl WebhookSenderConfig {
    pub fn into_sender_input(self) -> Result<Box<dyn SenderInput>, &'static str> {
        match self.input {
            SenderInputOpts::Queue(input_opts) => xwebhook_bridge_plugin_queue::into_sender_input(
                self.name,
                input_opts,
                self.transformation,
                self.output,
...
impl WebhookReceiverConfig {
    pub async fn into_receiver_output(self) -> std::io::Result<Box<dyn ReceiverOutput>> {
        match self.output {
            ReceiverOutputOpts::Queue(x) => xwebhook_bridge_plugin_queue::into_receiver_output(
                self.name.clone(),
                x,
                self.transformation.as_ref(),
            )
