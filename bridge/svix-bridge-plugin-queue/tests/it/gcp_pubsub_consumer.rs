...
use serde_json::json;
use xwebhook_bridge_plugin_queue::{
    config::{GCPPubSubInputOpts, QueueInputOpts},
    sender_input::QueueSender,
};
use xwebhook_bridge_types::{
    xwebhook::api::MessageIn, CreateMessageRequest, SenderInput, SenderOutputOpts, XwebhookOptions,
    XwebhookSenderOutputOpts, TransformationConfig, TransformerInput, TransformerInputFormat,
...
fn get_test_plugin(
    ...
    QueueSender::new(
        ...
        SenderOutputOpts::Xwebhook(XwebhookSenderOutputOpts {
            token: "xxxx".to_string(),
            options: Some(XwebhookOptions {
                server_url: Some(xwebhook_url),
                ..Default::default()
            }),
        }),
    )
...
/// Check to see if the xwebhook server sees a request.
#[tokio::test]
async fn test_consume_ok() {
    ...
    let mock = Mock::given(method("POST"))
        .respond_with(ResponseTemplate::new(202).set_body_json(json!({
          ...
          "payload": {
            "_XWEBHOOK_APP_ID": "app_1234",
            "_XWEBHOOK_EVENT_TYPE": "testing.things",
            "hi": "there",
          },
          "id": "msg_xxxx",
...
/// Push a msg on the queue.
/// Check to see if the xwebhook server sees a request, but this time transform the payload.
#[tokio::test]
...
/// Check that the plugin keeps running when it can't send a message to xwebhook
#[tokio::test]
async fn test_consume_xwebhook_503() {
    let client = mq_connection().await;
    let (topic, subscription) = create_test_queue(&client).await;
...
/// Check that the plugin keeps running when it can't send a message to xwebhook because idk, the servers are all offline??
#[tokio::test]
async fn test_consume_xwebhook_offline() {
    let client = mq_connection().await;
    let (topic, subscription) = create_test_queue(&client).await;
