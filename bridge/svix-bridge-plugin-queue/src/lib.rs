...
use xwebhook_bridge_types::{
    async_trait, xwebhook::api::Xwebhook, CreateMessageRequest, JsObject, TransformationConfig,
...
trait Consumer {
    ...
    /// The client to use when creating messages in xwebhook.
    ...
    fields(
        ...
        xwebhook_bridge_plugin.name = crate::PLUGIN_NAME,
        xwebhook_bridge_plugin.vers = crate::PLUGIN_VERS,
    )
    )]
    ...
    /// Parses the delivery as JSON and feeds it into [`create_xwebhook_message`].
    /// Will nack the delivery if either the JSON parse, transformation, or the request to xwebhook fails.
    ...
    async fn process(&self, delivery: DeliveryWrapper) -> std::io::Result<()> {
        ...
        match create_xwebhook_message(self.xwebhook_client(), payload).await {
            Ok(_) => {
                tracing::trace!("ack");
...
async fn create_xwebhook_message(
    xwebhook: &Xwebhook,
