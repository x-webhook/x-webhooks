...
pub use xwebhook;
use xwebhook::api::{MessageIn, PostOptions as PostOptions_, XwebhookOptions as _XwebhookOptions};
...
impl ReceiverInputOpts {
    pub fn path_id(&self) -> &str {
        match self {
            ...
            | ReceiverInputOpts::XwebhookWebhook { path_id, .. } => path_id,
        }
    }
}
...
pub struct XwebhookOptions {
    #[serde(default)]
    pub debug: bool,
    pub server_url: Option<String>,
}

impl From<XwebhookOptions> for _XwebhookOptions {
    fn from(XwebhookOptions { debug, server_url }: XwebhookOptions) -> Self {
        _XwebhookOptions { debug, server_url }
    }
}
...
pub enum SenderOutputOpts {
    Xwebhook(XwebhookSenderOutputOpts),
}

#[derive(Debug, Deserialize)]
pub struct XwebhookSenderOutputOpts {
    /// Xwebhook API token for the client.
    pub token: String,
    /// Options for the Xwebhook client.
    #[serde(default)]
    pub options: Option<XwebhookOptions>,
}
