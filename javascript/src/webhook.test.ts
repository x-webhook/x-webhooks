...
class TestPayload {
  ...
  public constructor(timestamp = Date.now()) {
    ...
    this.header = {
      "xwebhook-id": this.id,
      "xwebhook-signature": "v1," + this.signature,
      "xwebhook-timestamp": this.timestamp.toString(),
    };
  }
}
...
test("missing id raises error", () => {
  ...
  delete testPayload.header["xwebhook-id"];
...
test("missing timestamp raises error", () => {
  ...
  const testPayload = new TestPayload();
  delete testPayload.header["xwebhook-timestamp"];
...
test("invalid timestamp throws error", () => {
  ...
  const testPayload = new TestPayload();
  testPayload.header["xwebhook-timestamp"] = "hello";
...
test("missing signature raises error", () => {
  ...
  const testPayload = new TestPayload();
  delete testPayload.header["xwebhook-signature"];
...
test("partial signature throws error", () => {
  ...
  const testPayload = new TestPayload();
  testPayload.header["xwebhook-signature"] = testPayload.header["xwebhook-signature"].slice(0, 8);

  expect(() => {
    wh.verify(testPayload.payload, testPayload.header);
  }).toThrowError(WebhookVerificationError);

  testPayload.header["xwebhook-signature"] = "v1,";
...
test("multi sig payload is valid", () => {
  ...
  testPayload.header["xwebhook-signature"] = sigs.join(" ");
