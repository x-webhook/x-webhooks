...
class UserAgentMiddleware implements Middleware {
  public pre(context: RequestContext): Promise<RequestContext> {
    context.setHeaderParam("User-Agent", `xwebhook-libs/${VERSION}/javascript`);
...
export interface XwebhookOptions {
  debug?: boolean;
  serverUrl?: string;
}
...
export class Xwebhook {
  ...
  public constructor(token: string, options: XwebhookOptions = {}) {
    ...
    const baseUrl: string =
      options.serverUrl ?? regionalUrl ?? "https://api.webhook.openweb3.io";
...
class Endpoint {
  ...
  /**
   * @deprecated Since version 1.24.0. Use `headersUpdate` instead.
   */
  ...
  /**
   * @deprecated Since version 1.24.0. Use `headersPatch` instead.
   */
  public patchHeaders(
    appId: string,
    endpointId: string,
...
export interface WebhookRequiredHeaders {
  "xwebhook-id": string;
  "xwebhook-timestamp": string;
  "xwebhook-signature": string;
}
...
export class Webhook {
  ...
  public verify(
    ...
    let msgId = headers["xwebhook-id"];
    let msgSignature = headers["xwebhook-signature"];
    let msgTimestamp = headers["xwebhook-timestamp"];
  ...
  public sign(msgId: string, timestamp: Date, payload: string | Buffer): string {
    if (typeof payload === "string") {
      ...
      throw new Error(
        "Expected payload to be of type string or Buffer. Please refer to https://docs.webhook.openweb3.io/receiving/verifying-payloads/how for more information."
      );
    }
