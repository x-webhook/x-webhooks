...
namespace Xwebhook.Tests
{
    ...
    class TestPayload
    {
        internal const string XWEBHOOK_ID_HEADER_KEY = "xwebhook-id";
        internal const string XWEBHOOK_SIGNATURE_HEADER_KEY = "xwebhook-signature";
        internal const string XWEBHOOK_TIMESTAMP_HEADER_KEY = "xwebhook-timestamp";
        ...
        public TestPayload(DateTimeOffset timestamp)
        {
            ...
            headers.Set(XWEBHOOK_ID_HEADER_KEY, id);
            headers.Set(XWEBHOOK_SIGNATURE_HEADER_KEY, signature);
            headers.Set(XWEBHOOK_TIMESTAMP_HEADER_KEY, timestamp.ToUnixTimeSeconds().ToString());
        }
    }
    ...
    public class WebhookTests
    {
        ...
        public void TestMissingIdRaisesException()
        {
            ...
            testPayload.headers.Remove(TestPayload.XWEBHOOK_ID_HEADER_KEY);
        ...
        {
            var testPayload = new TestPayload(DateTimeOffset.UtcNow);
            testPayload.headers.Remove(TestPayload.XWEBHOOK_TIMESTAMP_HEADER_KEY);
        ...
        {
            var testPayload = new TestPayload(DateTimeOffset.UtcNow);
            testPayload.headers.Remove(TestPayload.XWEBHOOK_SIGNATURE_HEADER_KEY);
        ...
        {
            var testPayload = new TestPayload(DateTimeOffset.UtcNow);
            testPayload.headers.Set(TestPayload.XWEBHOOK_SIGNATURE_HEADER_KEY, "v1,g0hM9SsE+OTPJTGt/tmIKtSyZlE3uFJELVlNIOLawdd");
        ...
        public void TestUnbrandedSignatureIsValid()
        {
            ...
            unbrandedHeaders.Set("webhook-id", testPayload.headers.Get(TestPayload.XWEBHOOK_ID_HEADER_KEY));
            unbrandedHeaders.Set("webhook-signature", testPayload.headers.Get(TestPayload.XWEBHOOK_SIGNATURE_HEADER_KEY));
            unbrandedHeaders.Set("webhook-timestamp", testPayload.headers.Get(TestPayload.XWEBHOOK_TIMESTAMP_HEADER_KEY));
        ...
        public void TestMultiSigPayloadIsValid()
        {
            ...
            string[] sigs = new string[] {
                ...
                testPayload.headers.Get(TestPayload.XWEBHOOK_SIGNATURE_HEADER_KEY), // valid signature
                "v1,Ceo5qEr07ixe2NLpvHk3FH9bwy/WavXrAFQ/9tdO6mc=",
            };
            testPayload.headers.Set(TestPayload.XWEBHOOK_SIGNATURE_HEADER_KEY, String.Join(" ", sigs));
