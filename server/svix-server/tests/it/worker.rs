...
use crate::utils::{
    ...
    get_default_test_config, run_with_retries, start_xwebhook_server, start_xwebhook_server_with_cfg,
};
...
/// that no redirection is taken by the Xwebhook worker's `reqwest::Client`
...
async fn test_endpoint_disable_on_repeated_failure() {
    ...
    if !matches!(cfg.cache_type, xwebhook_server::cfg::CacheType::None) {
