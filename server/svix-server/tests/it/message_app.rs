...
use crate::utils::{
    ...
    get_default_test_config, start_xwebhook_server_with_cfg_and_org_id, IgnoredResponse, TestReceiver,
};
...
async fn test_app_deletion() {
    dotenvy::dotenv().ok();
    let cfg = xwebhook_server::cfg::load().expect("Error loading Configuration");
    let org_id = OrganizationId::new(None, None);
    let (client, _jh) =
        start_xwebhook_server_with_cfg_and_org_id(&get_default_test_config(), org_id.clone()).await;
