...
use crate::utils::{
    ...
    get_default_test_config, start_xwebhook_server, start_xwebhook_server_with_cfg,
    start_xwebhook_server_with_cfg_and_org_id, IgnoredResponse, TestClient, TestReceiver,
};
...
async fn test_recovery_expected_retry_counts() {
    ...
    let (client, _jh) = start_xwebhook_server_with_cfg(&cfg).await;
    ...
    for sec in [secret_1, secret_2, secret_3] {
        match sec {
            ...
            EndpointSecret::Asymmetric(key) => {
                let msg_id = last_headers.get("xwebhook-id").unwrap().to_str().unwrap();
                let timestamp = last_headers
                    .get("xwebhook-timestamp")
                    .unwrap()
                    .to_str()
                    .unwrap();
                let signatures = last_headers
                    .get("xwebhook-signature")
                    .unwrap()
                    .to_str()
                    .unwrap();
...
async fn test_endpoint_secret_encryption() {
    ...
    let (client, jh) = start_xwebhook_server_with_cfg_and_org_id(&cfg, org_id.clone()).await;
    let app_id = create_test_app(&client, "app1").await.unwrap().id;
    ...
    let cfg = get_default_test_config();
    let (client, _jh) = start_xwebhook_server_with_cfg_and_org_id(&cfg, org_id.clone()).await;
    client
        .get::<IgnoredResponse>(
            &format!("api/v1/app/{app_id}/endpoint/{}/secret/", ep.id),
...
async fn test_endpoint_stats() {
    ...
    let last_msg_time = {
        ...
        let cfg = get_default_test_config();
        let db = Arc::new(cfg);
        let db = xwebhook_server::db::init_db(&db).await;
...
async fn test_endpoint_header_key_capitalization() {
    let (client, _jk) = start_xwebhook_server().await;
...
async fn test_send_example() {
    ...
    let _: IgnoredResponse = client
        .post(
            ...
            serde_json::json!({"eventType": "xwebhook.ping"}),
            StatusCode::OK,
        )
        .await
        .unwrap();
