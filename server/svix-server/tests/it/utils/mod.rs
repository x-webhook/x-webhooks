...
pub fn get_default_test_config() -> ConfigurationInner {
    ...
    let cfg = xwebhook_server::cfg::load().unwrap();

    cfg.as_ref().clone()
}

pub async fn start_xwebhook_server() -> (TestClient, tokio::task::JoinHandle<()>) {
    start_xwebhook_server_with_cfg(&get_default_test_config()).await
}

pub async fn start_xwebhook_server_with_cfg(
    cfg: &ConfigurationInner,
) -> (TestClient, tokio::task::JoinHandle<()>) {
    start_xwebhook_server_with_cfg_and_org_id(cfg, OrganizationId::new(None, None)).await
}

pub async fn start_xwebhook_server_with_cfg_and_org_id(
    cfg: &ConfigurationInner,
    ...
    let jh = tokio::spawn(
        xwebhook_server::run_with_prefix(
