...
use xwebhook_server_derive::{aide_annotate, ModelIn, ModelOut};
use url::Url;
...
const XWEBHOOK_PING_EVENT_TYPE_NAME: &str = "xwebhook.ping";
const XWEBHOOK_PING_EVENT_TYPE_PAYLOAD: &str = r#"{"success": true}"#;
...
async fn send_example(
    ...
    let example = if data.event_type == EventTypeName(XWEBHOOK_PING_EVENT_TYPE_NAME.to_owned()) {
        XWEBHOOK_PING_EVENT_TYPE_PAYLOAD.to_string()
    } else {
        let event_type =
