...
impl FailureCacheKey {
    pub fn new(
        ...
        FailureCacheKey(format!("XWEBHOOK_FAILURE_CACHE_{org_id}_{app_id}_{endp_id}"))
    }
}
...
fn generate_msg_headers(
    ...
    if whitelabel_headers {
        ...
        headers.insert("xwebhook-id".to_owned(), id_hdr);
        headers.insert("xwebhook-timestamp".to_owned(), timestamp);
        headers.insert("xwebhook-signature".to_owned(), signatures_str);
    }
    headers.insert(
        "user-agent".to_owned(),
...
mod tests {
    ...
    // Tests endpoint signing keys -- expected values are fetched from the Xwebhook documentation for a
    ...
    fn test_generate_msg_headers_with_signing_key() {
        ...
        assert_eq!(
            actual.get("xwebhook-signature").unwrap(),
