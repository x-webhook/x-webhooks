...
use serde::Serialize;
use xwebhook::api::{MessageIn, Xwebhook, XwebhookOptions};
...
impl OperationalWebhookSenderInner {
    ...
    pub async fn send_operational_webhook(
        ...
        let xwebhook_api = Xwebhook::new(
            op_webhook_token,
            Some(XwebhookOptions {
        ...
        tokio::spawn(async move {
            // This sends a webhook under the Xwebhook management organization. This organization contains
            // applications which are each a regular organization. The recipient's OrganizationId is the
            // app UID to use.
            let resp = xwebhook_api
                .message()
                .create(
            ...
            match resp {
                ...
                Err(xwebhook::error::Error::Http(xwebhook::error::HttpErrorContent {
