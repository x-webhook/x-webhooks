...
async fn main() {
    ...
    match args.command {
        ...
        Some(Commands::GenerateOpenapi) => {
            let mut openapi = xwebhook_server::openapi::initialize_openapi();

            let router = xwebhook_server::v1::router();
            _ = aide::axum::ApiRouter::new()
                .nest("/api/v1", router)
                .finish_api(&mut openapi);

            let openapi = xwebhook_server::openapi::postprocess_spec(openapi);
            println!(
                "{}",
