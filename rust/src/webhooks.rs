// SPDX-FileCopyrightText: © 2022 Xwebhook Authors
...
const XWEBHOOK_MSG_ID_KEY: &str = "xwebhook-id";
const XWEBHOOK_MSG_SIGNATURE_KEY: &str = "xwebhook-signature";
const XWEBHOOK_MSG_TIMESTAMP_KEY: &str = "xwebhook-timestamp";
...
impl Webhook {
    ...
    pub fn verify<HM: HeaderMap>(&self, payload: &[u8], headers: &HM) -> Result<(), WebhookError> {
        let msg_id = Self::get_header(headers, XWEBHOOK_MSG_ID_KEY, UNBRANDED_MSG_ID_KEY, "id")?;
        let msg_signature = Self::get_header(
            headers,
            XWEBHOOK_MSG_SIGNATURE_KEY,
            UNBRANDED_MSG_SIGNATURE_KEY,
            "signature",
        )?;
        let msg_ts = Self::get_header(
            headers,
            XWEBHOOK_MSG_TIMESTAMP_KEY,
...
mod tests {
    ...
    use super::{
        Webhook, XWEBHOOK_MSG_ID_KEY, XWEBHOOK_MSG_SIGNATURE_KEY, XWEBHOOK_MSG_TIMESTAMP_KEY,
        UNBRANDED_MSG_ID_KEY, UNBRANDED_MSG_SIGNATURE_KEY, UNBRANDED_MSG_TIMESTAMP_KEY,
    };

    fn get_xwebhook_headers(msg_id: &str, signature: &str) -> HeaderMap {
        let mut headers = HeaderMap::new();
        headers.insert(XWEBHOOK_MSG_ID_KEY, msg_id.parse().unwrap());
        headers.insert(XWEBHOOK_MSG_SIGNATURE_KEY, signature.parse().unwrap());
        headers.insert(
            XWEBHOOK_MSG_TIMESTAMP_KEY,
    ...
    fn test_verify() {
        ...
        let signature = wh
            ...
            .unwrap();
        for headers in [
            get_xwebhook_headers(msg_id, &signature),
    ...
    fn test_verify_partial_signature() {
        ...
        for mut headers in [
            ...
            headers.insert(XWEBHOOK_MSG_SIGNATURE_KEY, partial.parse().unwrap());
    ...
    fn test_verify_incorrect_timestamp() {
        ...
        let mut headers = get_xwebhook_headers(msg_id, &signature);
        for ts in [
            OffsetDateTime::now_utc().unix_timestamp() - (super::TOLERANCE_IN_SECONDS + 1),
            OffsetDateTime::now_utc().unix_timestamp() + (super::TOLERANCE_IN_SECONDS + 1),
        ] {
            headers.insert(
                super::XWEBHOOK_MSG_TIMESTAMP_KEY,
    ...
    fn test_verify_with_multiple_signatures() {
        ...
        let headers = get_xwebhook_headers(msg_id, &multi_sig);
    ...
    fn test_no_verify_with_multiple_signatures() {
        ...
        let headers = get_xwebhook_headers(msg_id, &missing_sig);
