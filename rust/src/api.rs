...
impl Default for XwebhookOptions {
    fn default() -> Self {
        Self {
            debug: false,
            server_url: None,
...
impl Xwebhook {
    pub fn new(token: String, options: Option<XwebhookOptions>) -> Self {
        ...
        let cfg = Configuration {
            base_path,
            user_agent: Some(format!("xwebhook-libs/{CRATE_VERSION}/rust")),
...
#[cfg(test)]
mod tests {
    use crate::api::Xwebhook;

    #[test]
    fn test_future_send_sync() {
        fn require_send_sync<T: Send + Sync>(_: T) {}

        let xwebhook = Xwebhook::new(String::new(), None);
        let message_api = xwebhook.message();
